...
3. **Automatização**: Após a issue ser submetida, uma automação via GitHub Actions será acionada. Esta automação adiciona o evento ao arquivo `data/database.json` e gera um novo markdown com as informações do evento.
...
3. **Processamento Automático**: Assim como na adição de eventos, o cancelamento deles também é automatizado. Uma vez que a issue é submetida, o GitHub Actions irá atualizar o arquivo `data/database.json` para remover o evento e ajustar o markdown gerado.
